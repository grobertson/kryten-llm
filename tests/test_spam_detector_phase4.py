...
from kryten_llm.components.spam_detector import SpamDetector, SpamCheckResult
from kryten_llm.models.config import SpamDetectionConfig, MessageWindow
...
def test_penalty_max_duration_cap(detector, default_spam_config):
    ...
    if penalty_time:
        ...
        max_time = datetime.now() + timedelta(seconds=default_spam_config.max_penalty)
...
def test_cleanup_old_data_removes_expired(detector):
    ...
    recent_messages = [
        ts for ts in detector.user_messages[username]
...
def test_spam_check_performance_many_users(detector):
    ...
    start = time.time()
    result = detector.check_spam("new_user", "test", 1)
    elapsed = time.time() - start
