...
from kryten_llm.models.config import LLMConfig, ServiceMetadata


# Skip all tests in this module until KrytenClient mocking is implemented
pytestmark = pytest.mark.skip(reason="Requires KrytenClient mocking - Phase 5 components tested in unit tests")
...
async def test_service_discovery_on_startup(llm_service, nats_client):
    ...
    discovery_calls = [
        call_args for call_args in nats_client.publish.call_args_list
...
async def test_heartbeat_stops_on_shutdown(llm_service, nats_client):
    ...
    initial_heartbeat_count = len([
        c for c in nats_client.publish.call_args_list
        if c[0][0] == "kryten.service.heartbeat.llm"
    ])
...
async def test_metrics_in_shutdown_event(llm_service, nats_client):
    ...
    shutdown_calls = [
        c for c in nats_client.publish.call_args_list
        if c[0][0] == "kryten.lifecycle.llm.shutdown"
    ]
...
async def test_complete_lifecycle_flow(llm_service, nats_client):
    ...
    discovery_calls = [
        c for c in nats_client.publish.call_args_list
        if c[0][0] == "kryten.service.discovery"
    ]
    assert len(discovery_calls) == 1
...
async def test_discovery_disabled_in_config(base_config, nats_client, logger):
    ...
    startup_calls = [
        c for c in nats_client.publish.call_args_list
        if c[0][0] == "kryten.lifecycle.llm.startup"
    ]
    assert len(startup_calls) > 0
