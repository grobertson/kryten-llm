...
class TestPhase2PipelineIntegration:
    ...
    async def test_trigger_word_activates_response(self, llm_config_with_triggers: LLMConfig):
        ...
        from kryten_llm.components import TriggerEngine, PromptBuilder
    ...
    async def test_response_logger_records_all_fields(self, llm_config_with_triggers: LLMConfig, tmp_path):
        """Test that response logger records comprehensive data."""
        from kryten_llm.components import ResponseLogger, RateLimitDecision, TriggerEngine
        import json
    ...
    async def test_dry_run_checks_rate_limit_without_recording(self, llm_config_with_triggers: LLMConfig):
    ...
    async def test_full_9_step_pipeline_with_rate_limiting(self, llm_config_with_triggers: LLMConfig, tmp_path):
        ...
        from kryten_llm.components import (
            MessageListener, TriggerEngine, RateLimiter, PromptBuilder,
            LLMManager, ResponseFormatter, ResponseLogger
        )
        ...
        with patch('kryten_llm.components.rate_limiter.datetime') as mock_dt:
            ...
            mock_response.status = 200
            mock_response.json = AsyncMock(return_value={
                "choices": [{"message": {"content": "Robert Z'Dar is legendary!"}}]
            })
