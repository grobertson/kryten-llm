...
class ResponseValidator:
    ...
    def _check_relevance(
        ...
        user_words = {
            word.lower() 
            for word in re.findall(r'\b\w{4,}\b', user_message)
        }
        ...
        if not user_words:
            ...
            return ValidationResult(
                valid=True,
                reason="No keywords to check relevance",
                severity="INFO"
            )
        ...
        if context.get('current_video'):
            ...
            video_words = {
                word.lower() 
                for word in re.findall(r'\b\w{4,}\b', video_title)
            }
            if video_words:
