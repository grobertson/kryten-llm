...
class ContextManager:
    ...
    async def start(self, nats_client) -> None:
        ...
        channel = channel_config.channel if hasattr(channel_config, 'channel') else channel_config.get("channel", "unknown")
    ...
    async def _handle_video_change(self, msg: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if len(title) > self.config.context.max_video_title_length:
                ...
                logger.debug(f"Truncated video title to {self.config.context.max_video_title_length} chars")
