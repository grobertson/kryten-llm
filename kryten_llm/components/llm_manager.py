...
class LLMManager:
    ...
    def _get_provider_priority(self, preferred_provider: Optional[str]) -> List[str]:
        ...
        if self.config.default_provider_priority:
            ...
            priority_order = [
                name for name in self.config.default_provider_priority
    ...
    async def generate_response(self, request: LLMRequest) -> Optional[LLMResponse]:
        ...
        logger.info(
            f"Attempting {len(provider_order)} providers in order: {provider_order}"
        )
        ...
        logger.error(
            f"All {len(provider_order)} LLM providers failed. "
        ...
        for attempt in range(provider.max_retries + 1):  # +1 for initial attempt
            try:
                ...
                if attempt < provider.max_retries:
                    ...
                    logger.warning(
                        f"Provider {provider_name} failed after {attempt + 1} attempts"
                    )
                    raise
