...
class LLMService:
    ...
    def __init__(self, config: LLMConfig):
        ...
        # Phase 5 components
        self.health_monitor = None  # Initialized after NATS connection
        self.heartbeat_publisher = None  # Initialized after NATS connection
    ...
    async def _handle_chat_message(self, subject: str, data: dict) -> None:
        ...
        correlation_id = self._generate_correlation_id() if self.config.error_handling.generate_correlation_ids else None
        ...
        try:
            ...
            spam_check = self.spam_detector.check_spam(
                filtered["username"],
                filtered["msg"],
                is_bot_mention=(trigger_result.trigger_type == "mention"),
                user_rank=rank
            )
            ...
            if spam_check.is_spam:
                ...
                self.spam_detector.record_message(filtered["username"], filtered["msg"])
                return
            ...
            llm_request = LLMRequest(
                ...
                temperature=self.config.personality.temperature,
                max_tokens=self.config.personality.max_tokens,
                preferred_provider=trigger_result.preferred_provider if hasattr(trigger_result, 'preferred_provider') else None
            )
            ...
            for i, part in enumerate(formatted_parts):
                if self.config.testing.dry_run:
                    ...
                    logger.info(f"[{correlation_id}] Sent response part {i+1}/{len(formatted_parts)}")
                    sent = True
            ...
            self._handle_error(e, filtered.get("username", "unknown"), filtered.get("msg", ""), correlation_id)
    ...
    def _handle_error(self, error: Exception, username: str, message: str, correlation_id: str | None):
        ...
        if self.config.error_handling.enable_fallback_responses:
            logger.info(f"[{correlation_id}] Fallback responses enabled but not implemented in sync method")
